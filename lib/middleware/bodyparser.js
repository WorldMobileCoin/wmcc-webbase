  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (err instanceof SyntaxError)
        return res.send(500, parseErr(32700, 'Parse error'));

      return res.send(500, parseErr(32603, 'Internal error'));
    } finally {
      req.pause();
    }
...
function parseErr(code, msg) {
  return `{"result":null,"error":{"message": "${msg}","code":-${code}},"id":null}`;
}
