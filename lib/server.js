...
const Socket = require('wmcc-socket');
...
class Server extends Events {
  ...
  constructor(options) {
    ...
    this._keepAlive = this.config.keepAlive;
    this._noDelay = this.config.noDelay;
  }
  ...
  _bind() {
    ...
    this.http.on('connection', (socket) => {
      ...
      if (this._keepAlive)
        socket.setKeepAlive(this._keepAlive);
    ...
    this.http.on('request', async (hreq, hres) => {
      ...
      try {
        hres.setTimeout(this.config.timeout);
        hreq.on('error', () => {});
        hreq.pause();
    ...
    this.rpc.on('call', (res, cmd, query) => {
      this.handleCall(res, cmd, query);
      this.emit('call', res, cmd, query);
    });
  }
...
class ServerOptions {
  ...
  fromOptions(options) {
    ...
    if (options.keepAlive != null) {
      Assert(typeof options.keepAlive === 'boolean');
      this.keepAlive = options.keepAlive;
    }

    if (options.noDelay != null) {
      Assert(typeof options.noDelay === 'boolean');
